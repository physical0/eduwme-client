...
const StoryProblemBlocks: React.FC<{
  ...
  operation
    ...
    // Two-stage animation for multiply/divide
    const [currentStage, setCurrentStage] = useState<1 | 2>(1);

    // Two-stage alternating animation for multiply/divide
    useEffect(() => {
      if (operation === 'multiply' || operation === 'div') {
        const interval = setInterval(() => {
          setCurrentStage(prev => prev === 1 ? 2 : 1);
        }, 4000);
        return () => clearInterval(interval);
      }
    }, [operation]);

    // For multiplication - animated two-stage display
    const getMultiplicationDisplay = () => {
      return (
        <div className="flex flex-col items-center w-full relative" style={{ minHeight: '120px' }}>
          {/* Stage indicator with pulse animation */}
          <div className="flex gap-2 mb-2">
            <div className={`w-2 h-2 rounded-full transition-all duration-500 ${currentStage === 1 ? 'bg-cyan-500 scale-150 animate-pulse' : 'bg-gray-300 dark:bg-gray-600'}`} />
            <div className={`w-2 h-2 rounded-full transition-all duration-500 ${currentStage === 2 ? 'bg-cyan-500 scale-150 animate-pulse' : 'bg-gray-300 dark:bg-gray-600'}`} />
          </div>

          {/* Stage 1: Show groups with staggered animation */}
          <div className={`transition-all duration-700 ease-in-out w-full ${currentStage === 1 ? 'opacity-100 scale-100' : 'opacity-0 scale-95 absolute pointer-events-none'}`}>
            <div className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-300 mb-2 text-center">
              {num1} groups × {num2}
            </div>
            <div className="flex flex-row flex-wrap justify-center gap-2 sm:gap-3 max-w-[320px] mx-auto">
              {Array.from({ length: Math.min(num1, 8) }, (_, groupIndex) => (
                <div
                  key={`group-${groupIndex}`}
                  className="border-2 border-dashed border-green-500 dark:border-green-400 p-1 sm:p-1.5 rounded-lg animate-drop-in"
                  style={{ animationDelay: `${groupIndex * 100}ms` }}
                >
                  <div className="text-center text-[10px] sm:text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">
                    {groupIndex + 1}
                  </div>
                  <div className="grid grid-cols-4 gap-0.5">
                    {Array.from({ length: Math.min(num2, 12) }, (_, i) => (
                      <div
                        key={i}
                        className={`w-3 h-3 sm:w-4 sm:h-4 ${secondGroupColor} rounded-sm shadow-md animate-drop-in`}
              ...
              {num1 > 8 && (
                <div className="text-xs text-gray-500 dark:text-gray-400 self-center font-medium">
                  +{num1 - 8} more groups
                </div>
              )}
            </div>
          ...
          {/* Stage 2: Show result with animation */}
          <div className={`transition-all duration-700 ease-in-out w-full ${currentStage === 2 ? 'opacity-100 scale-100' : 'opacity-0 scale-95 absolute pointer-events-none'}`}>
            <div className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-300 mb-2 text-center">
              = ?
            </div>
            <div className="flex justify-center">
              <div className="grid grid-cols-8 gap-0.5 sm:gap-1 max-w-[200px]">
                {Array.from({ length: Math.min(result, 48) }, (_, i) => (
                  <div
                    key={i}
                    className="w-3 h-3 sm:w-4 sm:h-4 bg-purple-500 rounded-sm shadow-md animate-drop-in"
    ...
    // For division - animated two-stage display
    const getDivisionDisplay = () => {
      ...
      return (
        <div className="flex flex-col items-center w-full relative" style={{ minHeight: '120px' }}>
          ...
          {/* Stage 1: Show initial blocks with staggered animation */}
          <div className={`transition-all duration-700 ease-in-out w-full ${currentStage === 1 ? 'opacity-100 scale-100' : 'opacity-0 scale-95 absolute pointer-events-none'}`}>
            <div className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-300 mb-2 text-center">
              {num1} ÷ {num2}
            </div>
            <div className="flex justify-center">
              <div className="grid grid-cols-8 gap-0.5 sm:gap-1 max-w-[200px]">
                {Array.from({ length: Math.min(num1, 48) }, (_, i) => (
                  <div
                    key={i}
                    className={`w-3 h-3 sm:w-4 sm:h-4 ${firstGroupColor} rounded-sm shadow-md animate-drop-in`}
                    style={{ animationDelay: `${i * 20}ms` }}
                  />
                ))}
              </div>
          ...
          {/* Stage 2: Show divided groups with staggered animation */}
          <div className={`transition-all duration-700 ease-in-out w-full ${currentStage === 2 ? 'opacity-100 scale-100' : 'opacity-0 scale-95 absolute pointer-events-none'}`}>
            <div className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-300 mb-2 text-center">
              = ? groups{remainder > 0 ? ` with remainder ${remainder}` : ''}
            </div>
            <div className="flex flex-row flex-wrap justify-center gap-2 sm:gap-3 max-w-[320px] mx-auto">
              {Array.from({ length: Math.min(quotient, 8) }, (_, groupIndex) => (
                <div
                  key={`group-${groupIndex}`}
                  className="border-2 border-dashed border-blue-500 dark:border-blue-400 p-1 sm:p-1.5 rounded-lg animate-drop-in"
                  style={{ animationDelay: `${groupIndex * 100}ms` }}
                >
                  <div className="text-center text-[10px] sm:text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">
                    {groupIndex + 1}
              ...
              {quotient > 8 && (
                <div className="text-xs text-gray-500 dark:text-gray-400 self-center font-medium">
                  +{quotient - 8} more groups
                </div>
              )}
              {remainder > 0 && (
                <div
                  className="border-2 border-dashed border-yellow-500 dark:border-yellow-400 p-1 sm:p-1.5 rounded-lg animate-drop-in"
                  style={{ animationDelay: `${Math.min(quotient, 8) * 100}ms` }}
                >
                  <div className="text-center text-[10px] sm:text-xs font-semibold text-yellow-600 dark:text-yellow-400 mb-1">
                    R
                  </div>
                  <div className="grid grid-cols-4 gap-0.5">
                    {Array.from({ length: Math.min(remainder, 12) }, (_, i) => (
                      <div
                        key={i}
                        className="w-3 h-3 sm:w-4 sm:h-4 bg-yellow-500 rounded-sm shadow-md animate-drop-in"
                        style={{ animationDelay: `${Math.min(quotient, 8) * 100 + i * 30}ms` }}
                      />
                    ))}
                  </div>
                </div>
